...
<!-- <div class="hp_pc_none"> -->
  <select name="product_categories" onchange="location = this.value;">
  <?php
    $current_category_id = $_GET['category_id_big'] ?? ''; // 現在の URL からカテゴリー ID を取得
  ...
  <option value="<?= home_url('/products/?colorme_page=items') ?>" <?php echo (!$parentCategoryId) ? 'selected' : '' ?>>全ての製品</option>
  ...
  <?php if ($response) : ?>
    <?php foreach ($response['categories'] as $c) :
      ...
      ?>
      <!-- カテゴリー名 -->
      <option value="<?php echo $url->items([
          'category_id_big' => $c['id_big'],
      ]) ?>" <?php echo ($c['id_big'] == $parentCategoryId) ? 'selected' : '' ?>>
        <?php echo $c['name'] ?>
      </option>
    <?php endforeach; ?>
  <?php else : ?>
    <option disabled>商品カテゴリーが登録されていません。</option>
  <?php endif; ?>
</select>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    if (!<?php echo json_encode($categoryFound); ?>) {
      document.querySelector('select[name="product_categories"]').selectedIndex = 0;
    }
  });
</script>
